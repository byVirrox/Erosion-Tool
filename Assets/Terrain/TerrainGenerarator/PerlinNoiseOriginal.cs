...
public static class PerlinNoiseOriginal
{
    private static readonly int[] p;

    static PerlinNoiseOriginal()
    {
        int[] permutation = {
    ...
    {
        int X = (int)Math.Floor(x) & 255, // finde Einheitszelle, die den Punkt enthält
            Y = (int)Math.Floor(y) & 255,
            Z = (int)Math.Floor(z) & 255;

        x -= Math.Floor(x); // finde relative X,Y,Z-position des Punktes in der Zelle
        y -= Math.Floor(y);
        z -= Math.Floor(z);

        double u = fade(x), // berechne Übergangskurven für X, Y und Z
               v = fade(y),
               w = fade(z);

        int A = p[X] + Y, AA = p[A] + Z, AB = p[A + 1] + Z,     // hashe Koordinaten der 8 Eckpunkte der Zelle
